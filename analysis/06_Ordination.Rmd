...
# p value very small, significant relationship
...
Interpretation 2: I scaled the read counts to 600,000 because the max number of reads in a sample is 569992. I cannot perform a beta-diversity analysis yet, I must scale my reads to accurately compare the data.
...
Interpretation 3: I scaled the read counts to 2000 because after scaling the sequencing depth ranged from 1517 to 1607 across samples. I think this could be sufficient to perform a beta-diversity analysis and compare my data; however, the minimum sequencing depth is 1,607 reads, then the next lowest is 17,372 reads. I am tempted to remove this smallest one (16S0119C) since this is a 15,765 read difference.
...
3.	The **interaction between AgeRange and SampleSite** explains an additional ~`r round(sorensen_AgeRange_adonis_terms1$R2[3]*100, digits = 0)`% of the variation, with a pvalue of <`r sorensen_AgeRange_adonis_terms1$"Pr(>F)"[3]`, meaning the effect of SampleSite does not vary across turtle age.
4. Finally, the total residiuals of the model is ~`r round(sorensen_AgeRange_adonis_terms1$R2[4]*100, digits = 0)`% of the variation remains unexplained by these variables, which is a lot.

#### Biological takeaway:

AgeRange and SampleSite shape microbial beta diversity in this dataset based off of shared presence/absence of ASVs as it relates to the Sorensen dissimilarity. The null interaction effect suggests that spatial dynamics (*e.g.,* sample site) is homogeneous across the turtle microbiomes
...
**In the first model** where `Weight` is the final term, `Weight` explains only 2.8% of the variation in community composition after accounting for AgeRange and SampleSite. It is NOT statistically significant (p > 0.05), so `Weight` does not have an independent effect. *But in this model, weight is tested after AgeRange and SampleSite, so its effect is adjusted for their contributions.*

**In the second model** `Weight` now explains 4.6% of the variation, which is a bit higher! Here `Weight` is significant, indicating a strong association between turtle weight and community structure. However, weight is tested first, so it “gets credit” for shared variance with station and also date. 

These two models together show that: 

1. Weight is not important (not significant in both models).
2. AgeRange and SampleSite are important (significant in both models).
...
3.	The **interaction between AgeRange and SampleSite** explains an additional ~`r round(bray_AgeRange_adonis_terms1$R2[3]*100, digits = 0)`% of the variation, with a pvalue of <`r bray_AgeRange_adonis_terms1$"Pr(>F)"[3]`, meaning the effect of SampleSite does not vary across turtle ages.
4. Finally, the total residiuals of the model is ~`r round(bray_AgeRange_adonis_terms1$R2[4]*100, digits = 0)`% of the variation remains unexplained by these variables, which is a lot.
...
- The Sorensen model explains ~2% more total variation, suggesting that presence/absence captures more ecological structure (barely).
- The higher R^2^ for AgeRange (~`r round(sorensen_AgeRange_adonis_terms1$R2[1]*100, digits = 0)`% vs. ~`r round(bray_AgeRange_adonis_terms1$R2[1]*100, digits = 0)`%) and SampleSite (~`r round(sorensen_AgeRange_adonis_terms1$R2[2]*100, digits = 0)`% vs. ~`r round(bray_AgeRange_adonis_terms1$R2[2]*100, digits = 0)`%) in Sorensen indicates that abundance shifts across space are not more pronounced than presence/absence alone suggests.
- Residual variation is lower in Sorensen, further supporting its added sensitivity to ecologically meaningful abundance patterns.

*In other words:* 

Community composition is strongly influenced by AgeRange and SampleSite in both models, but the Sorensen model (presence/absence) explains more variation overall — especially for spatial effects. This suggests that rare species play a major role in structuring microbial communities in this system across age ranges and sample sites. 
...
**In the first model** where `Weight` is the final term, `Weight` explains only 2.7% of the variation in community composition after accounting for AgeRange and SampleSite, which was similar to the Sorensen results. Here, it IS statistically significant (p < 0.05), so `Weight` does have an independent effect. *But in this model, Weight is tested after AgeGroup and SampleSite, so its effect is adjusted for their contributions.*

**In the second model** `Weight` now explains 3.7% of the variation, which is slightly higher and a similar result to the Soresen PERMANOVA. `Weight` is also significant (p < 0.001), while `AgeRange` is not (p > 0.05). Here, weight is tested first, so it “gets credit” for shared variance with AGeRange and also SampleSite. 
...
2.	Slightly higher R^2^ in Sorensen indicates that AgeRange influences presence/absence (rare) patterns more than just abundance measured by the Bray-Curtis dissimilarity.
...
Therefore, AgeRange is a major driver in this system! Therefore, we can accept our hypothesis at the beginning of this file. :) Age's effect is robust across distance metrics (presence/absence and abundance), with more variation explained by presence/absence. And, some of its influence may be shared with weight, so term order matters. Finally, in Sorensen, AgeRange and SampleSite affects who is present, suggesting an effect on rare species. 
...
- **AgeRange** does not have a significant `betadisper()/permutest()` result. Therefore, the PERMANOVA results likely represent a true difference by age.
-  **SampleSite** does not have a significant `betadisper()/permutest()` result. Therefore, the PERMANOVA results likely represent a true difference by sample site.  
- **Weight** has a significant `betadisper()/permutest()` result. We cannot conclude for certain that our PERMANOVA results are truly because of weight.

Interpretation 7: AgeRange and SampleSite are major drivers in this system! We can still accept our hypothesis at the beginning of this file! Age's effect is robust across distance metrics (presence/absence and abundance), with more variation explained by presence/absence. Finally, in Sorensen, AgeRange and SampleSite affects who is present, suggesting an effect on rare species.
...
3. Sorensen highlights both spatial patterns more strongly because between the two axes more variation is explained (~20%) than Sorensen (~15%), suggesting that presence/absence differences among taxa matter in this system. We gain ~5% of more variation explained in the data in 2 axes without incorporating abundances.
...
1. The stress value for Sorensen is ~0.20 and for Bray-Curtis is ~0.22, which is too high for a strong result.
...
Interpretation 10: I think PCoA works best because the stress value for NMDS is too high. Additionally, the Sorensen PCoA is the only plot you could argue has some AgeRange differentiation. 
