...
metadata_df <- 
  ...
  # Fix First Column Name to sample_names
  dplyr::rename("sample_names" = "SampleID") %>%
...
Interpretation 1: This is a reasonable amount of chloroplast and mitochondria ASVs, there is a very small amount probably from contamination. 
...
neg_control_samples <- c("16SNEGCTRL")
...
neg_controlASV_physeq <- 
  ...
  # This dataset does not have any positive controls

# Take look 
neg_controlASV_physeq
sample_variables(neg_controlASV_physeq)
...
#Which ASVs have no variance = ASV_5798
asv_negControls_df %>%
  group_by(ASV, Sample_or_Control) %>%
  summarise(variance = var(Abundance)) %>%
  filter(variance == 0)

asv_negControls_df %>% filter(ASV == "ASV_5798")

#Get rid of ASV_5798 so Wilcox test will work
asv_negControls_df_filtered <- 
  asv_negControls_df %>%
  filter(ASV != "ASV_5798")


# Statistically test with filtered negative control df! 
...
```
From the plot above, it actually looks like we should include `ASV_0099` in our analyses.
...
The taxonomy of the ASV in question are: 

- `ASV_0099`: Genus is *Cupriavidus metallidurans*, which is a heavy metal resistant bacterium, frequently found in extreme conditions and sediments, and has been show to cause infections in humans. This doesn't make much sense with Mediterranean Loggerhead sea turtle microbiomes.

I will remove this ASV from my dataset as well. 

```{r asvs-to-keep-plot, fig.height=2.5, fig.width=4}
# Let's zoom in only on these two that we care about! 
asv_negControls_df %>%
  dplyr::filter(OTU == "ASV_0099") %>%
...
# Create a new vector with all ASVs
negControlASVs_rm <- dplyr::setdiff(neg_controlASVs_vec$ASV, c("nothing"))
...
Interpretation 2: My dataset did have a negative control (the tank water the turtles were kept in). Removing Cholorplasts/Mitochondria removed 6 ASVs. Removing negative controls removed 7 ASVs. This seems reasonable, as there are still 6632 ASVs. 

## 6. Positive Controls 

Interpretation 3: My dataset did not include a positive control so I cannot say anything about the accuracy of the sequencing run.
...
Interpretation 4: I have zero reverse compliments in my dataset!
...
Interpretation 5: My sequencing depth is around 100000 reads per sample. There are two outliers still with > 500000 reads. There are also some samples with (relatively) very few reads. I still think I have enough reads, but I will lose a handfull of samples with too few reads. 
...
min(sample_sums(noChlorosMitosNegControls_physeq))

#Keep samples with >500 reads
samples_to_keep <- seqSums_df$names[seqSums_df$TotalSeqs >= 500]

length(samples_to_keep)

raw_preprocessed_physeq <- 
  noChlorosMitosNegControls_physeq %>%
  subset_samples(., names %in% samples_to_keep)
